...
import { Droplets, Download, BarChart2, Loader2, Info, Eye } from "lucide-react";
...
const WaterManualDashboard = () => {
  ...
  const allManualDevices = useMemo(() => getDevicesByType('water'), []);
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      ...
      <header className="flex items-center justify-between mb-6 pb-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold tracking-tight">Eau — Manuel</h1>
          ...
          {!isViewer && (
            <>
              <Button 
                variant="outline" 
                size="sm"
                onClick={() => navigate('/comparison-manual?type=water')}
      ...
      {allManualDevices.length === 0 && !loading && (
        <div className="flex items-center justify-between p-4 rounded-xl bg-blue-500/5 border border-blue-500/20 mb-6 group transition-all hover:bg-blue-500/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Créez un appareil de type eau dans l'accueil pour commencer la saisie.</p>
            </div>
          </div>
          {!isViewer && (
            <Button 
              ...
              className="text-blue-400 hover:text-blue-400 hover:bg-blue-500/10 font-bold"
            >
              + Ajouter un appareil →
            </Button>
          )}
...
export default WaterManualDashboard;
